...
def	drawMatches(file1, file2, feat1, feat2):
	...
	for i in range(keypoints_left.shape[0]):
		if np.all(image1[int(keypoints_left[i, 1]), int(keypoints_left[i, 0])] == image1[int(keypoints_left[i, 1]), int(keypoints_left[i, 0])][0]):
			continue
		keypoints_left_new.append(keypoints_left[i])
	keypoints_left_new = np.array(keypoints_left_new)[: 65]

	for i in range(0, keypoints_right.shape[0]):
		if np.all(image2[int(keypoints_right[i, 1]), int(keypoints_right[i, 0])] == image2[int(keypoints_right[i, 1]), int(keypoints_right[i, 0])][0]):
			continue
		keypoints_right_new.append(keypoints_right[i])
	keypoints_right_new = np.array(keypoints_right_new)[: 65]
...
def	drawMatches2(file1, file2, feat1, feat2):
	...
	for i in range(keypoints_left.shape[1]):
		if np.all(image1[int(keypoints_left[1, i]), int(keypoints_left[0, i])] == image1[int(keypoints_left[1, i]), int(keypoints_left[0, i])][0]):
			continue
		keypoints_left_new.append(keypoints_left[:, i])
	keypoints_left_new = np.array(keypoints_left_new).T

	for i in range(keypoints_right.shape[1]):
		if np.all(image2[int(keypoints_right[1, i]), int(keypoints_right[0, i])] == image2[int(keypoints_right[1, i]), int(keypoints_right[0, i])][0]):
			continue
		keypoints_right_new.append(keypoints_right[:, i])
	keypoints_right_new = np.array(keypoints_right_new).T
	print(keypoints_right.shape)
	...
	cv2.imwrite('/home/udit/d2-net/extract_noRansac.jpg', im4)
